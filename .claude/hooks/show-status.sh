...
PHASES_ORDERED="understand research scope design decompose implement validate"
...
show_compact() {
    ...
    echo -e "${CYAN}${slug}${NC} | Phase ${phase_num}/7: ${BOLD}${current_phase}${NC}${task_info}"
}
...
show_full() {
    ...
    echo ""
    echo -e "  ${DIM}Phase:${NC}  ${BOLD}${CYAN}$current_phase${NC} ${DIM}(${phase_num}/7)${NC}"
    echo ""
    echo -e "  ${DIM}Progress:${NC} $(build_phase_progress "$current_phase" "$completed_phases")"
    echo -e "            ${DIM}1=understand 2=research 3=scope 4=design 5=decompose 6=implement 7=validate${NC}"
