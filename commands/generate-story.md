---
description: Generate detailed user stories from minimal input with assumption-first approach
---
...
2. **Delegate to story-generator agent**:
   ```
   Delegate to story-generator agent:

   Context: Generate a complete user story from raw input
...
4. Optionally, run `/story` to begin the engineering workflow
...
Once a user story is verified, you can seamlessly transition to implementation:

```
/story docs/stories/drafts/<story-slug>/user-story.md
```

This will:
1. Move the story from `drafts/` to the active story directory
2. Initialize the engineering workflow state
3. Begin Phase 1 (Understand) with the pre-generated story
...
5. **Generate testable criteria** - Every acceptance criterion should be verifiable in Given/When/Then format
